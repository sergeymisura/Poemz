 ...
 */
class HttpRequestExt extends CHttpRequest
{
	/**
	 ...
	 *
	 * @return  mixed  stdClass object with decoded JSON if the request is sent with 'application/json' content-type; raw POST data otherwise
	 */
	public function getPayload()
	{
		$payload = file_get_contents('php://input');
		return $this->getContentType() == 'application/json' ? json_decode($payload, true) : $payload;
	}

	/**
	 * Verifies if the connection is secure. Extends the default function to make it work properly behind the load balancer.
	 *
	 * @see CHttpRequest::getIsSecureConnection()
	 *
	 * @return  boolean  True if the client is connectied via HTTPS
	 */
